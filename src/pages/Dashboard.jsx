    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main className="forge-main forge-main-xl">
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, color: 'var(--subtle)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Dashboard</div>
          <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 28, color: 'var(--text)', margin: 0 }}>
            {user?.name ? `Hola, ${user.name} 👋` : 'Tu entrenamiento'}
          </h1>
          {user?.targetDate && daysLeft != null && (
            <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 11, color: daysLeft <= 7 ? 'var(--red)' : 'var(--subtle)', marginTop: 6 }}>
              {daysLeft > 0 ? `${daysLeft} días para la entrevista — ${daysLeft <= 7 ? '¡A full!' : 'buen ritmo.'}` : '¡El día es hoy!'}
            </p>
          )}
        </div>
        ...
        <div className="forge-grid-4 forge-section">
          <StatCard label="Progreso global" value={`${overallPct}%`} sub={`${ALL_TOPIC_IDS.length} temas`} accent />
          <StatCard label="Racha" value={streak > 0 ? `${streak}d` : '—'} sub={streak > 0 ? 'días seguidos' : 'Sin racha aún'} />
          <StatCard label="Días para entrevista" value={daysLeft != null ? (daysLeft > 0 ? `${daysLeft}d` : '¡Hoy!') : '—'} sub={user?.targetDate ? new Date(user.targetDate).toLocaleDateString('es-AR') : 'Sin fecha'} accent={daysLeft != null && daysLeft <= 7} />
          <StatCard label="Última sesión" value={lastSession ? `${lastSession.answers?.length || 0}` : '—'} sub={lastSession ? 'preguntas' : 'Sin sesiones aún'} />
        </div>

        {/* ── Progreso general ── */}
        <div className="forge-section">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
            <span style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 15, color: 'var(--text)' }}>Progreso general</span>
          ...
          <p style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, color: 'var(--subtle)', marginTop: 6 }}>
            {overallPct < 20 && 'Recién empezando. ¡El primer 20% es el más difícil!'}
            {overallPct >= 20 && overallPct < 50 && 'Buen ritmo. Seguí con los Tier 1.'}
            {overallPct >= 50 && overallPct < 80 && 'Más de la mitad. Ya se nota el avance.'}
            {overallPct >= 80 && '¡Casi listo! Repasá los temas débiles.'}
          </p>
        </div>
        ...
        <div className="forge-grid-2-wide forge-section">
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '20px 24px' }}>
            <Divider>Mapa de habilidades</Divider>
            <SkillRadar progress={progress} topics={TOPICS} />
          </div>

          {/* Streak */}
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '20px 24px' }}>
            <Divider>Actividad — últimos 35 días</Divider>
        ...
        <div className="forge-grid-2 forge-section">
          ...
          >
            ...
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 16, color: 'var(--text)' }}>Repaso rápido</div>
            <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, color: 'var(--subtle)', lineHeight: 1.5 }}>5 preguntas random de tus temas más débiles.</div>
            <div style={{ marginTop: 4, fontFamily: 'Space Mono, monospace', fontSize: 10, color: 'var(--primary)' }}>Empezar →</div>
          </div>
          ...
          >
            ...
            <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: 16, color: 'var(--text)' }}>Entrevista simulada</div>
            <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 10, color: 'var(--subtle)', lineHeight: 1.5 }}>Sin hints. Presión real. Reporte completo al final.</div>
            <div style={{ marginTop: 4, fontFamily: 'Space Mono, monospace', fontSize: 10, color: 'var(--primary)' }}>Configurar →</div>
          </div>
        </div>

        {/* ── Recomendado ── */}
        <div className="forge-section">
          <Divider>Recomendado para hoy</Divider>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, flex: 1, minWidth: 0 }}>
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 9, color: 'var(--primary)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>Próximo tema</div>
        ...
        {[1, 2, 3].map(tier => (
          <div key={tier} style={{ marginBottom: 40 }}>
            <Divider>{tier === 1 ? 'Tier 1 — Críticos' : tier === 2 ? 'Tier 2 — Importantes' : 'Tier 3 — Diferenciadores'}</Divider>
        ...
        {/* ── Roadmap ── */}
        <div>
          <Divider>Plan de 6 semanas</Divider>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 10 }}>
            {ROADMAP.map(w => {
              ...
              return (
                <div key={w.week} style={{ background: 'var(--surface)', border: `1px solid ${isCurrent ? 'var(--primary)' : 'var(--border)'}`, padding: 16 }}>
                  <div style={{ fontFamily: 'Space Mono, monospace', fontSize: 9, color: isCurrent ? 'var(--primary)' : 'var(--subtle)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 4 }}>
                    Semana {w.week}{isCurrent ? ' · EN CURSO' : ''}
                  </div>
      ...
      <footer className="forge-footer" style={{ marginTop: 40 }}>
        <span>DevForge beta</span>
        <button onClick={() => navigate('/')} style={{ background: 'none', border: 'none', color: 'var(--subtle)', cursor: 'pointer', fontFamily: 'Space Mono, monospace', fontSize: 10 }}>← Inicio</button>
      </footer>
